...
**版**: v0.3 (機構を正準モデルに差し替え)
**日付**: 2026-06-09 (v0.1: 2026-06-05, v0.2: 2026-06-08)
**著者**: Yuito (東大新領域人間環境学専攻 SCSLab 2026年秋 入学予定)
**コードネーム**: 真・PRISM (toy validation of intervention-response discriminative premise)
**ステータス**: pre-registration。§2(仮説・判定基準)/§14(decision tree)は確定・post-hoc 変更禁止。**v0.3 の主変更**: 自作 Model T/H(原典なし・T が不動点で死ぬ)を**原典のある正準モデル**に差し替え(Model T = Chiarella-Iori-Perelló 2009 の chartist+fundamentalist+noise、Model H = Kirman 1993 / Alfarano-Lux-Wagner 2008 の herding)。観測分離(§3.3)は維持。ファンダメンタルを雑音つき観測として開示(§3.1)し、それを fundamental 観測チャネル = B2 介入の第3軸に(§7.2)。経緯は `docs/research/abm_b2_intervention_survey.md`(世界 ABM 文献 29-agent 調査)。
...
- バーンイン: 1000 steps
- 測定: 10000 steps
- ファンダメンタル: 値は固定(p* = 100)。**v0.3: エージェントには雑音つき観測として開示**(ABIDES/Chiarella-Iori 流)。
  - 各 agent i は毎時刻 `ŷ_{i,t} = p* · exp(N(0, σ_f²))` を観測する(ŷ = perceived fundamental)。
  - σ_f は fundamental 観測チャネルの S/N を決める(σ_f → 0 で完全情報、大で無情報)。
  - この ŷ は fundamentalist 成分が読む観測であり、**同時に B2 介入の第3軸(fundamental masking、§7.2)**になる。
  - 注: v0.2 までの「非公開」は自作 T/H が fundamental を使わなかった前提。正準モデル(§3.2)は fundamentalist を含むため開示が必要。開示形を雑音つきにすることで B2 と整合させる。

### 3.2 機構仕様(v0.3: 正準モデル)

両 Model とも、agent ごとに離散行動 `a_{i,t} ∈ {-1,0,+1}` を出し、ED = Σ_i a_{i,t} で価格更新(§3.1)。判断の入力は**すべて観測ベクトル o_{i,t}(§3.3)経由**で、機構係数は観測層とは別(B2 ≠ A の核心)。**自作モデルの差し替え理由**: v0.2 までの自作 T/H は原典が無く、特に T は純決定論で初期 return ゼロから自走できず死んだ。v0.3 は原典のある正準モデルに置き、noise 成分で自走を保証する。

#### Model T: Chiarella-Iori 型(chartist-dominant、原典: Chiarella-Iori-Perelló 2009)

各 agent i は3成分のいずれか(母集団比率 α=(α_c, α_f, α_n)、chartist 優勢)。各成分は観測から内部信号を計算し連続需要 d_{i,t} を出す:

- **chartist**(価格履歴を読む): `trend_{i,t} = mean(r_{t-h_i:t})`(r=log-return)、`d = g_c · trend_{i,t}`。
- **fundamentalist**(雑音つき fundamental を読む): `m_{i,t} = log(ŷ_{i,t} / p_t)`(perceived mispricing、ŷ は §3.1)、`d = g_f · m_{i,t}`(平均回帰)。
- **noise**(自走の素): `d = ε`, `ε ~ N(0, σ_n)`。

行動: `a_{i,t} = sign(d_{i,t}) if |d_{i,t}| > θ_i else 0`。
ヘテロ性: 成分割当 ~ Categorical(α)、`h_i ~ DiscreteUniform[5, 50]`、`θ_i ~ Uniform[0, θ_max]`。
Free parameters (calibration target): α, g_c, g_f, σ_n, θ_max, σ_f, λ。

> chartist が読む価格履歴 = trend masking 軸、fundamentalist が読む ŷ = fundamental masking 軸(§7.2)。

#### Model H: 群れ(herding、原典: Kirman 1993 / Alfarano-Lux-Wagner 2008)

各 agent i は集約行動履歴(他者の行動)を読み、多数派へ確率的に同調する:

- `social_{i,t} = mean(ā_{t-h_i^s:t})`、`ā_τ = (1/N) Σ_j a_{j,τ-1}`(前期集約行動)。
- 行動: 確率 `p_i` で `a_{i,t} = sign(social_{i,t})`(同調)、確率 `1-p_i` で `{-1,0,+1}` 一様(独立切替=自走の素)。
- これは Kirman ant の2状態確率切替・voter 型社会的模倣の離散版。fundamentalist 成分は持たない(純 herding)。

ヘテロ性: `p_i ~ Uniform[p_min, p_max]`、`h_i^s ~ DiscreteUniform[5, 50]`。
Free parameters (calibration target): p の範囲、h^s の範囲。

> 社会信号 ā を読む = social masking 軸(§7.2)。H は fundamental を読まないので fundamental masking には(直接は)応答しないはず — これが軸×機構の弁別を生む設計。

#### 設計上の重要点(B2 ≠ A)

各成分は観測ベクトルから**内部で**信号を計算する。観測チャネルへの介入(§7)は「機構の入力源(o_{i,t})」を degrade するが、機構係数(g_c, g_f, p_i 等)には触れない。係数を 0 にするのは ablation = A であり禁止。これが B2 ≠ A の核心で、正準モデルでも観測層を機構から分離して保つことで担保する(文献整合: `docs/research/abm_b2_intervention_survey.md` §1-2)。

### 3.3 観測ベクトル(B2 が A に化けない設計)

各 agent に毎時刻渡される観測ベクトル o_{i,t}(**各要素が B2 介入の1軸に対応**、§7.2):

| 要素 | 次元 | 内容 | 読む成分 | 介入軸 |
|------|------|------|------|------|
| 価格履歴 (raw) | L = 100 | p_{t-L:t} の log-return 系列 | T.chartist | trend masking |
| 集約行動履歴 | L = 100 | ā_{t-L:t}(社会信号の raw source) | H | social masking |
| fundamental 観測 | scalar/short | `ŷ_{i,t} = p*·exp(N(0,σ_f²))`(雑音つき、§3.1) | T.fundamentalist | fundamental masking |
| 出来高履歴 | L = 100 | v_{t-L:t} | (未使用、将来) | — |

すべて raw(MA/momentum/rank などの加工特徴量は含まない)。各成分は raw 観測から内部で信号を抽出する(B2 ≠ A)。**観測チャネルを1本足すごとに介入軸が1本増える**——v0.3 では fundamental を足して 3 軸(trend/social/fundamental)。survey が挙げた他チャネル(order book、公開履歴窓、私的シグナル)は将来の軸候補。
...
### 7.2 介入軸(v0.3: 観測チャネル 1 本 = 介入軸 1 本)

**一般原則**: B2 の介入軸は「エージェントが読む観測チャネル」1つにつき 1 軸。チャネルを足せば軸が増える(§3.3 と1対1)。これは B2(観測)という1つの介入型の中の話で、A(機構係数)・B1(価格形成・市場構造)は別型の介入(§7.5 で対照群として並走)。

| 軸 | 介入対象(観測チャネル) | 期待される favored target |
|----|----|----|
| Trend masking | 価格履歴 p_{t-L:t} を degrade | Model T(chartist) |
| Social masking | 集約行動履歴 ā_{t-L:t} を degrade | Model H |
| Fundamental masking | fundamental 観測 ŷ_{i,t} を degrade | Model T(fundamentalist) |

各軸を独立に動かす(現状 3D intervention space、拡張可能)。各 model は各介入に応答するが、応答曲線の形が機構で異なる(と仮説する)。特に H は fundamental を読まないので fundamental masking に(直接は)応答しないはず——軸×機構の応答パターンが弁別シグナルになる。
...
### 7.5 対照介入(A / B1、v0.3 追加)

B2 が「機構を ablate せず入力源だけ degrade している」ことを清潔に示すため、対照として別型の介入を並走させる(survey 推奨):

- **A(機構係数)**: chartist g_c や herding p_i を直接弱める = ablation。**B2 と違い機構そのものを壊す**。
- **B1(価格形成・市場構造)**: tick_size / 取引税。PRISM が実データで試して日次 SF では null だった型(`docs/research/abm_b2_intervention_survey.md` §4)。

判定: **同一機構ペア (T,H) に対し、A・B1 では弁別できないが B2 では弁別できる**という非対称が取れれば、観測情報チャネル介入の固有の edge が立つ。A・B1 は v0 では最小実装(主結果は B2、A/B1 は robustness/対照)。
...
- Chiarella, C., Iori, G., Perelló, J. (2009). The impact of heterogeneous trading rules on the limit order book and order flows. Journal of Economic Dynamics and Control, 33(3), 525-537. (Model T の原典)
- Kirman, A. (1993). Ants, rationality, and recruitment. Quarterly Journal of Economics, 108(1), 137-156. (Model H の原典)
- Alfarano, S., Lux, T., Wagner, F. (2008). Time variation of higher moments in a financial market with heterogeneous agents: An analytical approach. Journal of Economic Dynamics and Control, 32(1), 101-136. (Model H の herding 解析)
- Byrd, D., Hybinette, M., Balch, T. (2019). ABIDES: Towards High-Fidelity Market Simulation. arXiv:1904.12066. (noisy fundamental observation の母体)
...
- v0.2 (2026-06-08): 留保 1/2 を default で解決。
  ...
  - ⚠ **未確定(留保 substance、後日議論)**: T* の具体固定値と選定根拠、Model H 探索空間の十分性、SF5/6 を verification に回す妥当性/閾値。これらは §4-§6 に「v0.2 暫定」として inline マーク済み(変更時はさらに版を切る)。
- v0.3 (2026-06-09): **機構を正準モデルに差し替え**(v0.2 の留保 substance とは別件)。
  - 自作 Model T/H(原典なし、T が純決定論で不動点に死ぬ)を棄却 → **Model T = Chiarella-Iori-Perelló (2009)** の chartist+fundamentalist+noise 混合、**Model H = Kirman (1993) / Alfarano-Lux-Wagner (2008)** の herding(§3.2)。noise 成分で自走を保証。
  - **ファンダメンタルを雑音つき観測として開示**(§3.1)。fundamentalist の入力かつ B2 第3軸。
  - 観測ベクトルに fundamental チャネル追加、**「観測チャネル1本=介入軸1本」**に一般化(§3.3, §7.2)。介入軸 2→3(trend/social/fundamental)。
  - 対照介入 A/B1 を §7.5 に明記(B2 の edge を非対称で示すため)。
  - §2/§14 不変。経緯: 世界 ABM 文献 29-agent 調査(`docs/research/abm_b2_intervention_survey.md`)。
  - 注: §5(calibration)は SF1-4 距離最小化の枠組みは不変だが、対象モデルが正準モデルに変わるため param 空間は §3.2 の新 free parameter に従う。