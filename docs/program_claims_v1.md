...
| 多重比較 | Holm、family = 少なくとも一方が観測チャネルを持つ全ペア × 全 scheme（v1.3 トリオで Genoa-ZI+ 採用時 {T,H,ALW,Genoa,CB,ZI} → 識別 13 ペア × 4 scheme = 52 検定／不採用時 {T,H,ALW,CB,ZI} → 8 ペア × 4 = 32 検定。family の定義は Genoa 採用 gate の帰結として事前登録）。(CB, FPZ-ZI) は等価予測側（TOST）、(H, ALW) は順序予測側（§2 (iv)）でそれぞれ識別 family から除外 | family-wise α = 0.05 |
